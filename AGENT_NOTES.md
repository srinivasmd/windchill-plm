...
| **Entity Set Names Differ** | DOMAIN_REGISTRY entity set names often don't match actual Windchill endpoints. Always verify with `query_entities(name, top=1)`. 8 mismatches fixed in commit 0917523: Baselines→BACReceivedDeliveries, ControlDocuments→TrainingRecords, Folders→ProjectContainers, WorkflowProcesses→WorkItems, LifecycleTemplates→WorkItems, CAPAs→Quality, Registrations→RegulatorySubmissions, UDIRecords→UDISuperSets, ServiceDocuments→SIMDocuments |
| **Search Method Required** | Every domain client needs `search_{entity}()` method for CLI integration. Without it, `zephyr search --domain X` fails with 400. Uses base class `self.search('{EntitySet}', term, domain=self.DOMAIN)`. |
| **search_* vs query_*: Different Parameters** | Domain clients have both `search_*()` and `query_*()` methods. `search_*()` uses `$search` (full-text, all fields). `query_*()` uses `$filter` (field-specific OData expression). CLI search command MUST call `search_*()`, not `query_*()` -- passing a search term to `query_*()` treats it as a `filter_expr`, causing 400 errors. |
| **Enum Properties Require /Value Suffix** | PTC.EnumType properties (State, Priority, Severity, Status, etc.) require `/Value` suffix in OData filters: `State/Value eq 'RELEASED'` (NOT `State eq 'RELEASED'`). Without `/Value`, server returns 400 "types not compatible". CLI auto-corrects common enum property names. |
| **BOM Child Details Require $expand=Uses** | PartUse links don't include child Number/Name by default. Must use `$expand=Uses` on the PartUse navigation to inline child Part details. Read child Number/Name from `item['Uses']`, NOT `item['Part']`. `get_bom()` defaults `expand_uses=True`. |
