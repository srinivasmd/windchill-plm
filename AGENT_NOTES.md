...
| **Entity Set Names Differ** | DOMAIN_REGISTRY entity set names often don't match actual Windchill endpoints. Always verify with `query_entities(name, top=1)`. 9 mismatches fixed: Baselines->BACReceivedDeliveries, Folders->ProjectContainers, CAPAs->Quality, etc. |
